...
A field graduates only if it clears its event-based gate: `condition` (the nowcast) graded as classification against the movement truth; `recovery_minutes` and its band graded per episode with positive CRPS skill over the duration-climatology baseline on enough incidents. `p_normal_in_H` stays shadow-only until the normal-condition (`normal_now`) backtest stratum shows it actually beats persistence there — or until it is re-scoped under the episode scorecard. Onset latency is reported but not gated — the alert feed is coincident-to-lagging by construction, so the model cannot lead it and isn't penalized for that.
