...
from training.prov import ArtifactFacts
...
def load_series_by_route(
    ...
    *,
    ...
    start_tick, end_tick_excl = aligned_window(start, end)
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.dry_run:
        dry_routes = {r: params_to_json(p) for r, p in per_route.items()}
        ...
        print(
            json.dumps(
                {
                    "global_prior": params_to_json(global_prior),
                    "routes": dry_routes,
    ...
    if args.skip_plausibility:
        ...
        live_doc = load_live_params(client, cfg.bucket)
        if live_doc is None:
            print(
    ...
    pending: list[DeferredPointer] = []
    params_pending: list[DeferredPointer] = []
