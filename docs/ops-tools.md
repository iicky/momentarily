...
| `alert_poll_latency` | Measure the poll latency the alerts-fetch cadence adds to the published condition: first-observed tick minus the alert's own `updated_at`, split onset/update/severe, plus `updated_at mod 300` (feed-clock test) and the liveness inter-tick clear-latency floor. | `uv run python -m training.alert_poll_latency --days 35 --json` | reads `archive/alerts/`, `archive/alerts_liveness/`; writes none |
