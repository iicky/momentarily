...
T0 sits at 10:00 ET (winter, UTC-5), with 5 hours of headroom before the next
tod_bin edge (15:00 ET, hour 15). Every synthetic series below stays inside
that window so a route's baseline lands in exactly one (route, tod_bin) cell
and a test's own arithmetic isn't at the mercy of a bin-boundary crossing.
"""
...
def _recovery(
    series: dict[tuple[str, int], int], baseline: dict[tuple[str, int], float]
):
...
def test_clean_route_yields_no_label():
    ...
    series = _flat("A", 40, 8)
    baseline = compute_baseline(series, min_samples=20)
...
def test_sustained_collapse_yields_chronic_label():
    ...
    baseline = compute_baseline(series, min_samples=20)
    assert baseline[("A", 2)] == 8.0  # median resists the 15/59 disrupted minority
...
def test_shuttle_low_baseline_not_flagged():
    ...
    baseline = compute_baseline(series, min_samples=20)
    assert baseline[("S", 2)] == 2.0
    assert baseline[("T", 2)] == 20.0
...
def test_was_at_baseline_true_when_lookback_window_is_all_normal():
    ...
    baseline = compute_baseline(_flat("A", 40, 8), min_samples=20)
