...
def test_long_window_archives_are_policed() -> None:
    # archive/vehicles/, archive/trip_updates/, archive/alerts_liveness/,
    # archive/health/, and archive/movement_census/ get a 3650d "effectively
    # keep" window rather than being left out of the table entirely — this
    # asserts they are still policed, not silently unbounded.
    client = _FakeClient(
        [
            ...
            "archive/movement_census/2010-01-01/1.json",  # >3650d old → expired
            "archive/movement_census/2026-06-01/2.json",  # 10d old → kept
        ]
    )
    expired = collect_expired(client, "b", NOW)  # type: ignore[arg-type]
    ...
    assert expired["archive/movement_census/"] == [
        "archive/movement_census/2010-01-01/1.json"
    ]
