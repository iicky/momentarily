...
# --- PROV-JSON sidecar (training.prov wiring) ---------------------------------

_FEED_FACTS = FeedFacts(
    version="20260807-H-test",
    sha256="d" * 64,
    start="2026-08-07",
    end="2026-09-04",
)
...
def test_write_prov_publishes_pointer_and_versioned_snapshot() -> None:
    """write_prov mirrors the other sidecars: a live pointer + an immutable
    versioned snapshot, byte-identical, that parses as PROV-JSON and names every
    published artifact as an entity generated by the run."""
    fake = _FakeS3()
    corpus = CorpusStats(
        start_tick=100,
        end_tick=200,
    ...
    artifacts = [
        ArtifactFacts("params", "state/params/v42.json", derived_from_manifest=True),
        ArtifactFacts(
            "scheduled_headway",
            "state/scheduled_headway/v42.json",
            derived_from_feed=True,
        ),
    ]
    versioned = write_prov(
        cast("S3Client", fake),
        "test-bucket",
        trained_at=42,
        started_at=1,
    ...
    assert versioned == f"{VERSIONED_PROV_PREFIX}v42.json"
    assert fake.objects[PROV_KEY] == fake.objects[versioned]

    doc = json.loads(fake.objects[versioned])
    # PROV-JSON shape: keyed maps, prov: prefixed relation records.
    assert set(doc["activity"]) == {"mmly:run/42"}
    generated = {r["prov:entity"] for r in doc["wasGeneratedBy"].values()}
    assert "mmly:state/params/v42.json" in generated
    assert "mmly:state/scheduled_headway/v42.json" in generated
    # The feed entity is used by the run and derives the scheduled-headway artifact.
    feed_ids = [e for e in doc["entity"] if "gtfs/" in e]
    assert feed_ids
    used_entities = {r["prov:entity"] for r in doc["used"].values()}
    assert feed_ids[0] in used_entities
    derived = {r["prov:usedEntity"] for r in doc["wasDerivedFrom"].values()}
    assert feed_ids[0] in derived


def test_write_prov_omits_feed_when_unavailable() -> None:
    """feed=None (fetch failed) => no feed entity and no feed-used edge, the
    grounding rule applied by omission."""
    fake = _FakeS3()
    corpus = CorpusStats(
        start_tick=0, end_tick=1, n_observations=1, input_blake3="b" * 64
    )
    write_prov(
        cast("S3Client", fake),
        "test-bucket",
        ...
        started_at=1,
        corpus=corpus,
        artifacts=[ArtifactFacts("params", "state/params/v7.json")],
        feed=None,
    )
...
def test_write_params_records_gtfs_feed_manifest_version_and_prov_ref() -> None:
    """The params doc gains the GTFS feed identity (version + digest), the
    vehicle-inclusive manifest marker, and a prov_ref — without disturbing the
    existing provenance/training_corpus blocks."""
    fake = _FakeS3()
    corpus = CorpusStats(
        start_tick=100,
        end_tick=200,
    ...
    write_params(
        ...
        trained_at=99,
        feed=_FEED_FACTS,
        prov_ref="state/prov/v99.json",
    )
    doc = json.loads(fake.objects[PARAMS_KEY])
    assert doc["gtfs_feed"] == {
        "version": _FEED_FACTS.version,
        "sha256": _FEED_FACTS.sha256,
        "start": _FEED_FACTS.start,
        "end": _FEED_FACTS.end,
    }
    assert doc["prov_ref"] == "state/prov/v99.json"
    tc = doc["training_corpus"]
    assert tc["input_manifest_version"] == 2
    assert tc["n_vehicle_keys"] == 9
    assert tc["n_input_versions"] == 3
    assert tc["input_blake3"] == "c" * 64
    # Existing block untouched.
    assert set(doc["provenance"]) == {"code_sha", "dirty", "producer"}


def test_main_publishes_prov_sidecar_and_threads_prov_ref(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Fixture path (no live publish): main() fetches the feed once, threads the
    same feed + a prov_ref into the artifact writers, and calls write_prov with
    the run's start time and every published artifact."""
    cfg = _r2_config()
    fake_client = cast("S3Client", _FakeS3())
    series = {"R1": _quiet(10)}
    corpus = CorpusStats(
    ...
    captured_headway: dict[str, Any] = {}
    captured_prov: dict[str, Any] = {}
    ...
    def _fake_fetch_gtfs_feed() -> tuple[bytes, FeedFacts]:
        return b"zip-bytes", _FEED_FACTS

    def _fake_write_params(*args: Any, **kwargs: Any) -> str:
        captured_params.update(kwargs)
        return "state/params/v55.json"

    def _fake_write_service_baseline(*args: Any, **kwargs: Any) -> int:
        return 4

    def _fake_write_segment_params(*args: Any, **kwargs: Any) -> int:
        return 3

    def _fake_write_segment_dwell(*args: Any, **kwargs: Any) -> tuple[int, Any]:
        from types import SimpleNamespace

        return 0, SimpleNamespace(n_cells_own=0, n_cells_route=0, n_cells_system=0)

    def _fake_write_scheduled_headway(*args: Any, **kwargs: Any) -> int:
        captured_headway.update(kwargs)
        return 5

    def _fake_write_prov(*args: Any, **kwargs: Any) -> str:
        captured_prov.update(kwargs)
        return "state/prov/v55.json"
    ...
    monkeypatch.setattr("training.eval.load_predictions", _fake_load_predictions)
    monkeypatch.setattr("training.train_em._movement_baseline", _fake_movement_baseline)
    monkeypatch.setattr("training.train_em.fetch_gtfs_feed", _fake_fetch_gtfs_feed)
    monkeypatch.setattr("training.train_em.write_params", _fake_write_params)
    monkeypatch.setattr(
        "training.train_em.write_service_baseline", _fake_write_service_baseline
    )
    monkeypatch.setattr(
        "training.train_em.write_segment_params", _fake_write_segment_params
    )
    monkeypatch.setattr(
        "training.train_em.write_segment_dwell", _fake_write_segment_dwell
    )
    monkeypatch.setattr(
        "training.train_em.write_scheduled_headway", _fake_write_scheduled_headway
    )
    ...
    # Feed threaded into params provenance and into scheduled_headway (same bytes).
    assert captured_params["feed"] is _FEED_FACTS
    assert captured_headway["feed_zip_bytes"] == b"zip-bytes"
    prov_ref = captured_params["prov_ref"]
    assert prov_ref.startswith(VERSIONED_PROV_PREFIX)
    assert prov_ref.endswith(".json")
    assert captured_headway["prov_ref"] == prov_ref

    # write_prov called with the run start time and the published artifacts.
    assert captured_prov["feed"] is _FEED_FACTS
    assert isinstance(captured_prov["started_at"], int)
    names = {a.name for a in captured_prov["artifacts"]}
    assert names == {
        "params",
        "service_baseline",
        "segment_params",
